...
export async function saveDataUrl(folder: string, dataUrl?: string|null){
  ...
  const blob = await put(key, bytes, { access: 'public', addRandomSuffix: false, contentType: `image/${ext}` });
  return blob.url;
}

export async function savePdf(name: string, buffer: Buffer){
  const key = `pdfs/${name}.pdf`;
  const blob = await put(key, buffer, { access: 'public', addRandomSuffix: false, contentType: 'application/pdf' });
  return blob.url;
}
